...
void ofApp::setup() {
	...
	this->cap.open("D:\\MP4\\Pexels Videos 2053855.mp4");
...
void ofApp::update() {
	...
	for (int i = 0; i < this->rect_images.size(); i++) {
		...
		int frames_index = ofMap(ofNoise(this->cv_rects[i].x * 0.0005, this->cv_rects[i].y * 0.0005, ofGetFrameNum() * 0.02), 0, 1, 0, this->number_of_frames);
